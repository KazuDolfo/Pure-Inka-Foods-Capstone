...
export class TopBar {
  ...
  // Use CartService aggregated items (ensures name/price/total exist)
  ...
  // Build image URL similarly to ProductCard.getImageUrl
  getImageUrl(image?: string): string {
    if (!image) return 'assets/pure-inka-logo.png';

    // Si ya tiene la ruta construida (/public/..., http... o assets/...)
    if (image.startsWith('/') || 
    ...
    // Si es solo el nombre del archivo, asumir que está en uploads/products/ via proxy
    if (!image.includes('/')) {
